...
const StateReview1 = () => {
  ...
  /**
   * input에 입력된 값을 그대로 출력
   * @param {*} e : 이벤트 객체(발생한 이밴트 관련 정보가 담긴 객체)
   */
  const inputStringHandler = (e) => {
    // 상태 변수 str값을 입력된 값으로 변경
  ...
  const enterHandler = (e) => {
    ...
    if(e.key === 'Enter'){ // 엔터 키가 입력된 경우
      ...
      // 상태변수 StrList의 값을 변경
      setStrList([...strList, str]); // e.target.value 혹은 str

      e.target.value = ''; // input에 작성된 값 삭제
      setStr(''); // 상태 변수에 '' 대입 -> 상태 변수값 변경시 리랜더링
    }
  ...
  return(
    <div>
      ...
      <div>
        <label htmlFor="inputSring">문자열 입력 : </label>
        <input
          type="text"
          id = "inputString" 
          onChange={inputStringHandler}//{}안에 함수 작성
      ...
      <ul>
        {/* <li>문자열 입력 후 엔터 입력시 ul태그에 누적</li> */}
